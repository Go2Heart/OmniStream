...
DINOV3_BASE_URL="/mnt/vision_user/yibinyan/VFM/facebook/dinov3"
...
def _make_dinov3_vit(
    ...
    **kwargs,
    ...
    if pretrained:
        ...
        if is_url(weights) and not weights.startswith('file://'):
            # 远程URL使用torch.hub下载
            state_dict = torch.hub.load_state_dict_from_url(url, map_location="cpu", check_hash=check_hash)
        else:
            # 本地文件直接加载，避免缓存
            local_path = Path(weights).expanduser().resolve()
...
def dinov3_vitb16(
    ...
    **kwargs,
    ...
    weights = "/mnt/vision_user/xjl/checkpoints/dinov3/dinov3_vitb16_pretrain_lvd1689m-73cec8be.pth"
    if "hash" not in kwargs:
...
def dinov3_vitl16(
    ...
    **kwargs,
    ...
    #     if "hash" not in kwargs:
    weights = "/mnt/vision_user/yibinyan/VFM/facebook/dinov3/dinov3_vitl16_pretrain_lvd1689m-8aa4cbdd.pth"
    if "hash" not in kwargs:
        kwargs["hash"] = "8aa4cbdd"
...
def dinov3_vith16plus(
    ...
    **kwargs,
    ...
    weights = "/mnt/vision_user/yibinyan/VFM/facebook/dinov3/dinov3_vith16plus_pretrain_lvd1689m-7c1da9a5.pth"
    return _make_dinov3_vit(
        img_size=224,
        patch_size=16,
        in_chans=3,
